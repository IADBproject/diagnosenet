...
class IO_Functions:
    ...
    def _read_file(self, file_path) -> None:
        if file_path[-3:] == "npy":
            items_corpus =np.load(file_path)
    ...
    def _write_npy_file(self, data, file_path) -> None:
    ...
    def _write_npy_batches(self, path: str, data: DataSplit,
                        batch_size: int, dataset_name: str,) -> None:
        fnumber = 1
        for i in range(len(data.inputs)):
            if i % batch_size == 0:
                X_fname = str(path+"X-"+dataset_name+"-"+str(fnumber)+'.npy')
                y_fname = str(path+"y-"+dataset_name+"-"+str(fnumber)+'.npy')
                fnumber += 1

                if os.path.exists(X_fname) == False:
                    np.save(X_fname,data.inputs[i:i+batch_size])
                else:
                    pass

                if os.path.exists(y_fname) == False:
                    np.save(y_fname, data.targets[i:i+batch_size])
                else:
                    pass
    ...
    def _write_npy_batches_worker(self, path: str, data: DataSplit, devices_number: int,
                        batch_size: int, dataset_name: str,) -> None:
        """
        IO_Function to baching the datse over the number of working for NFS.
        """
        ...
        def batching_woker(mod_index, X_worker_name, y_worker_name, batch_inputs, batch_targets):
            ...
            for j in range(len(batch_inputs)):
                ...
                if batch_file_num == math.ceil(worker_samples/float(batch_size)):
                    ...
                    batch_range_end = mod_index+j+batch_size
                if j % batch_size == 0:
                    X_fname = str(X_worker_name+"-"+str(batch_file_num)+'.npy')
                    y_fname = str(y_worker_name+"-"+str(batch_file_num)+'.npy')
                    batch_file_num += 1

                    if os.path.exists(X_fname) == False:
                        np.save(X_fname,data.inputs[mod_index+j:batch_range_end])
                    else:
                        pass

                    if os.path.exists(y_fname) == False:
                        np.save(y_fname, data.targets[mod_index+j:batch_range_end])
                    else:
                        pass
